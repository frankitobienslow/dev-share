...
exports.getDesarrolladorById = async (req, res) => {
    try {
        const usuario = await Desarrollador.findByPk(req.params.id);
        if (usuario) {
            res.status(200).json(usuario);
        } else {
            res.status(404).json({ message: 'Deasarrollador  no encontrado' });
        }
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
